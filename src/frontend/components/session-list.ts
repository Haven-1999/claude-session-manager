...
export interface SessionListHandlers {
  ...
  onMoveSession: (sessionId: string, tagId: string) => void;
  onCreateTag: (name: string) => Promise<void>;
  onRenameTag: (id: string, name: string) => Promise<void>;
  onDeleteTag: (id: string, action: 'move_uncategorized' | 'delete_sessions') => Promise<void>;
}
...
export class SessionList {
  ...
  render(sessions: any[], tags: TagSummary[], activeId: string | null): void {
  ...
  private createSessionItem(s: any, activeId: string | null, tags: TagSummary[]): HTMLElement {
    ...
    el.addEventListener('dblclick', () => {
      const nameEl = el.querySelector('.session-name') as HTMLElement;
  ...
  private showContextMenu(e: MouseEvent, session: any, tags: TagSummary[]): void {
  ...
  private renderManagementMode(tags: TagSummary[], sessions: any[]): void {
    // Header
    const header = document.createElement('div');
    ...
    `;
    const addInput = addRow.querySelector('.tag-add-input') as HTMLInputElement;
    ...
    // List tags
    for (const tag of tags) {
      const sessionCount = sessions.filter((s: any) => (s.tagId || UNCATEGORIZED_TAG_ID) === tag.id).length;
  ...
  private startInlineRename(row: HTMLElement, tag: TagSummary, tags: TagSummary[], sessions: any[]): void {
    row.innerHTML = `
    ...
    `;
    const input = row.querySelector('.tag-rename-input') as HTMLInputElement;
    input.focus();
    input.select();
  ...
  private showDeleteConfirm(row: HTMLElement, tag: TagSummary, sessionCount: number, tags: TagSummary[], sessions: any[]): void {
  ...
  private loadExpandState(): void {
    try {
      ...
      if (saved) this.expandState = JSON.parse(saved);
    } catch {}
  }
